...
void oneParameter(String Parametro, float param){
  byte* b = dueFlashStorage.readAddress(4); // byte array which is read from flash at adress 4
  Configuration configurationFromFlash; // create a temporary struct
  memcpy(&configurationFromFlash, b, sizeof(Configuration)); // copy byte array to temporary struct

  if (Parametro == "central"){
    configurationFromFlash.a = param;
  }
  else if (Parametro == "left"){
    configurationFromFlash.b = param;
  } 
  else if (Parametro == "right"){
    configurationFromFlash.c = param;
  }
  else if (Parametro == "pressF"){
    configurationFromFlash.d = param;
  }
  else if (Parametro == "pressM"){
    configurationFromFlash.e = param;
  }
    // write configuration struct to flash at adress 4
  byte b2[sizeof(Configuration)]; // create byte array to store the struct
  memcpy(b2, &configurationFromFlash, sizeof(Configuration)); // copy the struct to the byte array
